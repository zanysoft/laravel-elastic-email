...
class Email
{
    ...
    public function GetStatus($transactionID, $showFailed = false, $showSent = false, $showDelivered = false, $showPending = false, $showOpened = false, $showClicked = false, $showAbuse = false, $showUnsubscribed = false, $showErrors = false, $showMessageIDs = false) {
        return $this->client->Request('email/getstatus', array(
     ...
     * @param $encodingType                                0 for None, 1 for Raw7Bit, 2 for Raw8Bit, 3 for QuotedPrintable, 4 for Base64 (Default), 5 for Uue  note that you can also provide the text version such as "Raw7Bit" for value 1.  NOTE: Base64 or QuotedPrintable is recommended if you are
     *                                                     validating your domain(s) with DKIM.
     ...
     * @param array                                        <string, string> $headers Optional Custom Headers. Request parameters prefixed by headers_ like headers_customheader1, headers_customheader2. Note: a space is required after the colon before the custom header value. headers_xmailer=xmailer:
     *                                                     header-value1
     * @param string $postBack                             Optional header returned in notifications.
     * @param array                                        <string, string> $merge Request parameters prefixed by merge_ like merge_firstname, merge_lastname. If sending to a template you can send merge_ fields to merge data with the template. Template fields are entered with {firstname},
     *                                                              {lastname} etc.
    ...
    public function Send(
        ...
        return $this->client->Request('email/send', $arr, "POST", $attachmentFiles);
    }
    ...
    public function Status($messageID) {
        return $this->client->Request('email/status', array(
            'messageID' => $messageID
        ));
    }
    ...
    public function View($messageID) {
        return $this->client->Request('email/view', array(
            'messageID' => $messageID
        ));
    }
